<?php
	...
	class LoginClass
	{
		...
		*/
		...
		{
			$to = $post['email'];
			$subject = "Activatie mail";
			$message = "Geachte heer/mevrouw <b>".$post['firstname']." ".
											   $post['infix']." ".
											   $post['lastname']."</b><br>";
												
			$message .= '<style>a { color:red;}</style>';
			$message .= "Hartelijk dank voor het registreren op Bonsai Online."."<br>";
			...
			$message .= "klik <a href='http://localhost/2014-2015/InlogRegistratieSysteem/Les 1/index.php?content=activate&id=".$id."&email=".$post['email']."&password=".$password."'>hier</a> om uw account te activeren"."<br>";
			
			$message .= "U kunt dan vervolgens een nieuw wachtwoord instellen."."<br>";
			$message .= "Met vriendelijke groet,"."<br>";
			$message .= "Faaraz Rasolzadeh"."<br>";

		
			$headers = 'From: no-reply@videotheek'."\r\n";
			$headers .= 'Reply-To: webmaster@videotheek'."\r\n";
			$headers .= 'Cc: admin@videotheek.nl'."\r\n";
			$headers .= 'Bcc: accountant@videotheek.nl'."\r\n";
